...
It is recommended that you wrap your ->get() or helper methods listed below with exception handling for the 'RavenToolsAPIException' class to avoid uncaught exceptions in the event of invalid user input or provider latency. Exceptions can be thrown in '->setMethod()', '->curl()', '->check_required()' and '->parse_response()' as of this writing.
...
* $Raven->addKeyword( $keyword, $domain )
* $Raven->removeKeyword ( $keyword, $domain )
